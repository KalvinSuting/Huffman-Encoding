...
public class HuffmanTree2{
   ...
   public HuffmanTree2(int[] count){	//create a treeNode for each frequency.
   ...
   public HuffmanTree2(BitInputStream input	){
	   int n = input.readBit();
	   overallRoot = explore(overallRoot, n, input);
   ...
   public void assign(String[] codes){
	  for(int i = 0; i < codes.length; i++){
		  codes[i] = explore(overallRoot, "", i);
	  }
   }
   
   private String explore(HuffmanNode root, String s, int level){
	   if(root.isLeaf()){
